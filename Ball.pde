...
class Ball {
  ...
  Ball (float s, float mspeed, color c, Team t) {
    this.size = s;
  ...
  void ballThrown() {
    ...
    if (this.team == red) //switch teams of the ball so it can determine if it hits someone on the other team.
      this.team = blue;
  ...
  boolean hitPlayer() {
    ...
    for (Player p : this.team.playersIn) {
      float distance = dist(p.pos.x, p.pos.y, this.pos.x, this.pos.y);
