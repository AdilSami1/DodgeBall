...
{
    public static int numBallsPerTeam = 3; //balls per team
    public static int numPlayersPerTeam = 5; //players per team
    //bug: rare, but when a player gets out very close to the middle line, the ball is unreachable

    public Ball[] balls= new Ball[numBallsPerTeam*2]; 
    public Team red, blue;
    public int green;
    public int yellow;
    public static int radiusMultiplier = 5;
    public int time = 0;
    ...
    red = new Team(this, "red team", numPlayersPerTeam, color(255, 80, 50));
    blue = new Team(this, "blue team", numPlayersPerTeam, color(50, 80, 255));
    
    for (int i=0; i<numPlayersPerTeam; i++) { //adding players to teams
    
        //Player(team, size, throwLevel, catchLevel)
        Player redPlayer = new Player(this, red, random(40f, 45f), random(0.5f, 1.0f), random(20f, 50f), random(2f, 3f));
        red.allPlayers[i] = redPlayer;
        red.addPlayer(red.allPlayers[i]); //adds player to playersIn list
        
        Player bluePlayer = new Player(this, blue, random(40f, 45f), random(0.5f, 1.0f), random(20f, 50f), random(2f, 3f));
    ...
    for (int i=0; i<numBallsPerTeam; i++) {  //creating dodge balls
        ...
        Ball redsBall = new Ball(this, random(15, 18), random(5,7), color( random(255), random(255), random(255)), red);
    ...
    for (int i=0; i<numBallsPerTeam; i++) {  //creating dodge balls
        ...
        Ball bluesBall = new Ball(this, random(15, 18), random(5,7), color( random(255), random(255), random(255)), blue);
